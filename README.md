...
| **Component Groups** | Group components (a component can belong to SEVERAL groups), assign groups to one or multiple pages, aggregated group status = worst of its members. Groups are explicitly global (`is_global`) — removing a group from a page does NOT re-add it | Agrupar componentes (un componente puede pertenecer a VARIOS grupos), asignar grupos a una o múltiples páginas, estado agregado del grupo = el peor de sus miembros. Los grupos son globales de forma explícita (`is_global`) — quitar un grupo de una página no lo vuelve a agregar |
...
| **Component Dependencies** | Cascade status changes across related components — many components can depend on a shared hub; changes propagate transitively (A→B→C) and cycles are rejected | Cascada de cambios de estado entre componentes relacionados — varios componentes pueden depender de un hub compartido; los cambios se propagan transitivamente (A→B→C) y se rechazan ciclos |
