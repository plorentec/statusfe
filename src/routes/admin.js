...
router.post('/pages', (req, res) => {
  const { name, slug, description, status, is_public } = req.body;
  if (!name || !slug) {
  ...
  const page = pages.create({ name, slug, description, status, is_public });
...
router.put('/pages/:id', (req, res) => {
  ...
  pages.update(req.params.id, { name, slug, description, status, is_public });
  if (req.body.component_ids) {
